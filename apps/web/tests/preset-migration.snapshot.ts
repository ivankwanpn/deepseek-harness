/** Cold V2 restoration mounts the shipped PTC preset and publishes only a current-generation successor. */
...
import { normalizeSessionSnapshots, persistedSessionFilename } from '@deepseek-ai/dsh-session-snapshot'
...
describe.skipIf(webSnapshotMode() === 'record')('historical preset restoration through the Web Host', () => {
  it.each([false, true])('resumes code as PTC (selection events=%s)', async (withSelections) => {
    ...
    try {
      ...
      const successor = join(dirname(predecessor), persistedSessionFilename(SESSION_FORMAT_VERSION, 'zstd'))
      ...
      expect((await readdir(dirname(predecessor))).filter(name => name.endsWith('.jsonl.zstd')).sort())
        .toEqual(['session.v2.jsonl.zstd', persistedSessionFilename(SESSION_FORMAT_VERSION, 'zstd')])
    } finally {
