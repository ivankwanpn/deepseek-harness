...
ACP stdout comparisons omit only JSON-RPC `session/update` notifications tagged `config_option_update`, on both the captured and committed sides. Model discovery is best-effort and may finish after session close starts; [ACP bridge tests](../../acp/acp/tests/bridge.spec.ts) own notification contents and nonblocking prompt/close behavior. Configuration responses and all other frames remain compared. Record and refresh write the same projection; replay never rewrites stdout fixtures. The [decision](../../../.agents/notes/implemented/testing/2026-09-13-acp-snapshot-topology-notifications.md) explains the deterministic comparison.
