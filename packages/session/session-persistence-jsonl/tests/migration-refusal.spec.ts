/** Durable EOF refusals preserve historical generations and never fall back from a native current-format record. */
...
async function store(version: 2 | typeof SESSION_FORMAT_VERSION, compression: JsonlCompression, rows: readonly SessionFormatJsonObject[]) {
...
describe.each(modes)('EOF migration refusal ($compression, $access)', ({ compression, access }) => {
  ...
  it.each(nativeRefusals)('refuses native current-format $name instead of falling back to readable V2', async ({ tail, diagnostic }) => {
    ...
    const path = await store(SESSION_FORMAT_VERSION, compression, [...nativePrefix, tail])
    const original = await observe(path)
