...
目录按普通会话缓存，拉取走 single-flight；scope 创建时的 `warm` 钩子预热该会话的缓存项，三种转发信号丢弃缓存项：`agent-preset/selected` 丢弃该会话这一项（目录属于 preset，而空会话可能在预热之后才切换），`skills/change` 丢弃全部缓存项，因为 skill 注册表的失效通知不带过滤条件，`connection/reset` 清空全部缓存，因为宿主目录可能跨连接代际不同。由目录寻址的可继续 subagent 在客户端解析为没有 skill 候选，因为现有 skill RPC 要求会话已挂载；查看其持久化历史不得激活它。列表 RPC 使用插件注册时捕获的根上下文连接；草稿 chip 视觉由 `lexicon` 扫描派生。
