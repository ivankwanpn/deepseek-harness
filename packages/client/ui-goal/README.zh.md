...
Web GUI 的 goal 界面同时显示持久 goal 状态及当前的进程本地激活状态，供用户编辑、暂停、恢复或清除 goal；被拒绝的变更所产生的错误会内联显示。它把持久的 `/goal` 运行显示为 `Command input` 气泡，让用户或模型发出的命令在重新加载后仍然可见。它还提供编辑新建 goal 部署默认值的「目标上限」行。goal 创建仍不归本包。除 `minimal` 外，随附的 Web preset 都会向 agent（智能体）提供 `/goal`。
...
只要挂载了设置传输与「通用设置」区段，本插件还会提供一行「目标上限」，用于编辑领域解析的三个 `goal` 部署默认值（[运行时默认值](../../goal/goal/README.zh.md#runtime-defaults)）：轮次上限、token 预算与活跃工作预算。字段在每次输入被接受时写入；把它清空会清除该覆盖值，让字段重新继承部署的组合条目。
