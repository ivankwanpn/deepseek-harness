...
- **跟随改名之后的那次安装，会记在市集所列之名下。** 目录标记、面板、row id 与内容目录都以 manifest 所列之名为键，因此卸载也需要那个名字——市集已弃用的旧名并不是一个已安装的插件。跟随改名时 `install` 会报告这次改名，新名字就是从这里得知的。
...
- 磁盘上的能力比对**漏掉了 `commands/`**，因此每个带该目录的插件每次 sync 都永远报告「磁盘上的能力已变更」，原因是安装记录了一个比对永远找不到的能力。它还手工拼那份清单，并且 `mcp` 取的是可用 server 而不是载体文件，于是 `.mcp.json` 里没有任何可挂载项时也会如此。现在它改读 `detectCapabilities`——安装当初用来记录的那同一个探针。
- 插件被**卸载**后，市集为它插入的那一行**仍留在 patch layer 里**：拥有关系是从「想要的列」推导的，而卸载掉的插件不在其中任何一列里，于是 loader 继续为一个已经不存在的内容挂载 `mcp-client`。现在拥有关系就是 `marketplace:` id 命名空间，这正是「记录已随插件消失的行仍能被认作我们的」所依赖的东西。
- 跟随已发布的改名时，插件被记在**呼叫者输入的那个旧名**之下，于是以 manifest 所列之名作为键的目录标记会报告该条目未安装，而再安装一次就会产生重复。通知把旧名讲了两遍也是同一个原因。现在记录、它的 row id、内容目录与每一条讯息都以该条目之名命名。
- `marketplace add` 让抓取失败**以 rejection 逃逸**，与 `install` 不同，于是打错的 registry 会以未处理的 rejection 抵达 `bin.ts` 的 `process.exit(await …)`，而不是这个介面本来要打印的一行诊断。
