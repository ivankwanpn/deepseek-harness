...
- **An install that followed a rename is recorded under the marketplace's name.** The catalog marker, the panel, the row ids and the content directory all key on the name the manifest lists, so uninstalling needs that name — the alias the marketplace retired is not an installed plugin. `install` reports the rename when it follows one, which is where the new name is learned.
...
- the on-disk capability comparison **omitted `commands/`**, so every plugin shipping one reported "capabilities changed on disk" on every sync forever, because the install recorded a capability the comparison could never find. It built its list by hand and read `mcp` from the usable servers rather than from the carrier file, so a `.mcp.json` declaring nothing mountable did the same. It now reads through `detectCapabilities`, the probe the install recorded with.
- a row the marketplace inserted for a plugin that was later **uninstalled stayed in the patch layer**: ownership was derived from the desired rows, and an uninstalled plugin appears in none of them, so the loader kept mounting an `mcp-client` for content that was gone. Ownership is the `marketplace:` id namespace now, which is what lets a row whose record went with the plugin still be recognized as ours.
- following a published rename recorded the plugin under the **alias the caller typed**, so the catalog's marker — keyed on the name the manifest lists — reported the entry as not installed and a second install duplicated it. The notice named the alias twice for the same reason. The record, its row id, the content directory and every message name the entry now.
- `marketplace add` let a fetch failure **escape as a rejection**, unlike `install`, so a mistyped registry reached `bin.ts`'s `process.exit(await …)` as an unhandled rejection rather than as the one-line diagnostic this surface exists to print.
