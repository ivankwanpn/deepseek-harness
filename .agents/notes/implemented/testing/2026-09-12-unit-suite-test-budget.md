...
Those deaths are also visible to the coverage gate, which reports four of them in an instrumented run. That gate fails for two further reasons this change does not own. `packages/client/ui-settings-marketplace/src/client/MarketplaceSettingsTab.tsx` reports 93.67% lines and 89.58% branches even when only its own package's suites run, and neither that file nor its specs differ from master, so the per-file gate was already red there. The five `packages/llm/llm-deepseek/src` files that fail inside the partitioned gate measure 100% when that package alone is run instrumented with the gate's own environment variables, so their loss sits in the partition inventory or merge rather than in the suites; two gate runs reproduced identical percentages, which rules out the worker deaths as the cause of either.
