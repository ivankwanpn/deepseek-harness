...
这些死亡对覆盖率闸同样可见，一次插桩运行报告了其中四次。该闸另外两个原因之一已经关闭：`packages/client/ui-settings-marketplace/src/client/MarketplaceSettingsTab.tsx` 曾在一个与其 spec 都与 master 无异的文件上报告 93.67% 行与 89.58% 分支，现在测得 100%——它的案例覆盖空目录、被拒绝的读取与重试、卸载取消，以及一个不是 Error 的拒绝，而唯一不可达的防御分支带上了 `/* v8 ignore next -- reason */`。另一个则是一个**丢失的 spec 文件**，而不是合并故障：盘点是带着 `packages/llm/llm-deepseek/tests/adapter.spec.ts` 的——以闸自身环境重跑协调器所用的 `vitest list --filesOnly`，它在 1209 个文件之中——而 `assignWeightedPartitions` 把它留在了一个 309 个文件的分桶里，然而那次闸日志列出了它十一个同类 spec 却从未列出它，同时闸自己的统计只交代了那 1209 个文件中的 1208 个。未覆盖的函数正是该 spec 所覆盖的那些（`providerRejectedNormalizedImage`、`providerRejectedFileId`、`detailNamesFileId`），因此运行它的 fork 死掉了，把它的结果与覆盖率一并带走。协调器现在会拒绝那一条分区，而不是在没有它的情况下照常合并：`assertNoDiedWorkers` 读取每条分区的输出尾部里 Vitest 的 worker 死亡标记，失败并点名该分区，于是一个丢失的文件不再能以「某个无辜原始码文件的逐文件阈值失败」的形式重新浮现。fork 死亡本身是**间歇性**而非决定性的——四次闸形状的运行里，同一份文件被丢掉两次、干净跑完两次，而无论是同一分桶的四条并发分区还是单条插桩分区都无法复现它。经由闸自己的入口（`pnpm run test:coverage:partitioned`）运行完成时没有死亡、没有丢失文件、也没有阈值失败；裸跑 `tsx scripts/run-coverage-partitions.ts` 不能替代，因为协调器通过 `npm_execpath` 解析 pnpm，缺少它时盘点是空的。
