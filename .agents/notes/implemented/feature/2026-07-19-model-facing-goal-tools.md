...
`get_goal()` returns the current goal or `null`. A non-null result contains the compare-and-set id and revision, objective, durable phase, admitted and maximum goal rounds, any blocker reason, plus the process-local activation observation. It also reports a configured token or active-work ceiling, the spend against it, and which ceiling is exhausted, each omitted when the goal has no such budget ([resource budgets](2026-09-13-goal-resource-budgets.md)). `create_goal(objective, max_goal_tokens?, max_goal_work_ms?)` creates one long-running same-session objective, inheriting the deployment's continuation round cap rather than naming one ([round-cap ownership](2026-09-13-deployment-owned-goal-round-cap.md)). `update_goal(goal_id, revision, action, objective?, max_goal_rounds?, max_goal_tokens?, max_goal_work_ms?, blocked_reason?)` supports `edit`, `pause`, `resume`, `complete`, and `blocked`; replacement fields are valid only for `edit`, while a non-empty `blocked_reason` is required only for `blocked` and persists under the stable `model-reported` code. A durable paused goal rejects `resume` with `GOAL_TOOL_RESUME_PAUSED`; the user-facing command or Web control owns that transition. The executor treats exact empty-string optional fields and a zero `max_goal_rounds`, `max_goal_tokens`, or `max_goal_work_ms` as strict-schema fillers: they count as omitted, an edit still requires at least one meaningful replacement, and all non-filler values retain the action restrictions. A ceiling the deployment cannot meter is refused with `GOAL_BUDGET_UNMETERED`, and no tool argument removes an existing budget; the domain's explicit `null` and `/goal` remain the clearing paths.
...
- Deployment policy also owns the continuation round cap: the model-facing create surface names none, so a deployment's configured default bounds every goal a model creates.
