...
`tests/catalog.spec.ts` covers the read: one row per entry with installability and the installed marker decided by the Host, containment of an unreadable registration, ordered so the unreachable one is visited first, registration order across marketplaces, and the empty registry. `tests/search-command.spec.ts` drives the CLI command itself and pins both halves of the containment — the readable registration's match on stdout and the failed one's reason on stderr — together with the output format, the `[installed]` marker, and the no-match line. `tests/gateway.spec.ts` pins the five published methods, the install failure codes, the read-only refusal, the catalog answering on a read-only deployment, a successful install returning what it recorded alongside the status, and the two arms of the pin opt-in: the same unpinned request is refused without `allowUnpinned` and installs the commit the resolver named with it. `tests/install-reasons.spec.ts` asserts the `reason` field of all four refusals rather than their messages, so a reworded message cannot change a wire code. `tests/install-unpinned.spec.ts` drives the resolve-then-pin path at the install level: `resolveRefSha` and `fetchPlugin` are the only stubbed modules, and the test asserts that the entry's declared ref is what was resolved, that the returned commit is what the install records in state, and that the default arm refuses before it asks any remote. What stays uncovered is `resolveRefSha`'s own `git ls-remote` call and its HEAD-to-`main`-to-`master` fallback, which need a real remote.
