...
位于 `packages/goal/goal/` 的 `@deepseek-ai/dsh-goal` 通过 `ctx.goals` 管理一个当前的同会话目标。目标包含带品牌的 id、目标描述、持久化阶段、比较并交换修订号、`maxGoalRounds`，以及可选的 `maxGoalTokens` 与 `maxGoalWorkMs` 上限（[资源预算](2026-09-13-goal-resource-budgets.zh.md)）。`defaultMaxGoalRounds` 是经过校验的部署配置，默认值为 `256`；`defaultMaxGoalTokens` 与 `defaultMaxGoalWorkMs` 同样是经过校验的配置，默认不设上限；`create()` 在变更前于内部将三者一并解析为完整值，而不会把解析过程暴露为额外的服务动词。
