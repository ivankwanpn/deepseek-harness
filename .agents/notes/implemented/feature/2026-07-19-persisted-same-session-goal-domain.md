...
`@deepseek-ai/dsh-goal` in `packages/goal/goal/` owns one current same-session goal through `ctx.goals`. A goal has a branded id, objective, durable phase, compare-and-set revision, `maxGoalRounds`, and the optional `maxGoalTokens` and `maxGoalWorkMs` ceilings ([resource budgets](2026-09-13-goal-resource-budgets.md)). `defaultMaxGoalRounds` is a validated deployment setting with default `256`, and `defaultMaxGoalTokens` and `defaultMaxGoalWorkMs` are validated settings that default to no ceiling; `create()` materializes every one internally before mutation rather than exposing resolution as another service verb.
