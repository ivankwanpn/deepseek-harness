...
The goal-limits work changed the durable shape of a goal change payload while the checkout still wrote Session format 3, and the latest released format is also 3. `pnpm run verify-persistence-changes` therefore refuses to acknowledge the change and `doc-sync` stays 40/41:
...
Eight further changes are same-version additions and need no edge: `tokensUsed` and `workMsUsed` on the message sources of `agent/inbox/spliced`, `session/title-llm-request`, and `user/message`, plus `tokensAtCreate` and `workMsAtCreate` on `goal/change`.

The runtime already reads the new shape tolerantly — the goal fold admits the three ceilings as optional and `decodeCeiling(undefined)` returns `null`, so a record written before budgets existed replays as unbounded. What is missing is the durable declaration: without an adjacent edge, a build predating the change refuses a format-3 record whose `maxGoalRounds` is `null`, and the persistence gate has no version transition to acknowledge.

## Proposal

Add the adjacent Session format edge **v3 → v4** and make the writer write v4. v4 is the goal-limits payload as the current writer already emits it; v3 keeps its released meaning.
...
This is lossless with respect to current replay: a v3 record migrated to v4 carries the same meaning it already had when read by the tolerant decoder. The difference is that a v4 record states its ceilings explicitly instead of relying on absence.

### Packages and wiring

- New package `packages/session/session-format-v3-to-v4` following the `session-format-v2-to-v3` layout: the v4 codec, a stateful Stage migration (`transformEvent`, `transformRun`, `finish`), the target-header validator, and the target restorer.
...
- `SESSION_FORMAT_VERSION` moves from 3 to 4 in the core Session types alongside the declarations, then `pnpm run gen-session-format-catalog` regenerates the catalog. The generated file is never hand-edited.
- Before the writer changes, the archive command freezes the complete v3 persistence schema as the bilingual `docs/persistence-changes/historical-formats/v3.*` reference, so every integer below the writer keeps its own document.

### Current-version consumers

Update every consumer that means "current": Session creation and restoration, JSONL filename selection and publication, the catalog's current encoder and restorer, projection-cache generation identity, replay and snapshot normalization, and the TypeScript and Python SDK recordings. Literal historical versions stay in released codecs and historical fixtures. Read paths keep their existing behavior: header-only listing reads no bodies, a historical read may return the migrated in-memory artifact without publishing, and a write publishes only the final current successor.

### Snapshot successors and SDK projections

Each historical snapshot keeps its file and gains a v4 successor generated from the target version's canonical filenames; predecessors stay byte-identical and parent/child roles stay contiguous. Both SDK projections are re-recorded for the new generation. The snapshot corpus policy is updated for the current generation while direct-edge, multi-hop, packed-row, retry/failure, and shipped-profile coverage stay.
...
- `pnpm run verify-persistence-changes` reports the v3 → v4 transition and passes with all eleven changes acknowledged.
...
- `pnpm run verify-session-format-catalog` and the cookbook's focused Vitest baseline pass, with the new edge's test path added to that run.
- The Linux lanes pass: `pnpm run test:snapshot` (including the corpus) and `pnpm run test:expected`, with each historical generation's successor present and each predecessor byte-identical.
...
- The owning note [Released Session formats migrate through stateful streaming stages](../../implemented/architecture/2026-08-31-released-session-format-migrations.md) is updated rather than duplicated; bilingual pairing is re-recorded; `test:docs`, `doc-sync`, and `lint` pass.
...
- **The Windows checkout cannot run the snapshot lanes.** Stages 5 and 6 depend on a Linux environment; on Windows the lane fails by policy, so the work is only complete once the Linux lanes have actually run.
