...
查阅 V4 写入器变更之前捕获的 V3 持久化类型。系统提示词头是一条持久化的 `system/message` 事件，PTC 派发事件取代 V2 的 code-dispatch 名称，请求 header 不再携带 `data.header.system`。本快照保留 V3 的声明类型，包括没有任何已发布 tag 捕获的同版本载荷追加。
...
来源为 tag `dsh-v0.1.6-alpha.1`，其写入器常量为 3，本工作树的祖先合并了该 tag 的源码树；[预发布归档](../releases/README.zh.md)捕获的 tag 止于 `dsh-v0.1.5-rc.2`。本快照是 V4 写入器变更之前该工作树的完整 V3 目录，因此有四个载荷根类型保留了没有任何 tag 捕获的后续同版本追加：`goal/change` 快照成员 `maxGoalTokens` 与 `maxGoalWorkMs` 及 `tokensAtCreate`、`workMsAtCreate` 计数器，以及 `agent/inbox/spliced`、`session/title-llm-request`、`user/message` 上的消息来源 `tokensUsed` 与 `workMsUsed`。
...
- `SessionHeader.version` 解析为字面量 3；物理 `JsonlHeaderLine.version` 仍为 `number`。逻辑与物理 header 均要求 `isSeeded`，没有 `seedLength`。
- 系统提示词头是一条持久化的 `system/message` 事件，`request/header.data.header` 不声明 `system` 成员。`tool/ptc-dispatch` 与 `tool/ptc-dispatch-start` 取代 V2 的 `code-dispatch` 名称，V2 预设 id `code` 在 header 与选择中产出 `ptc`。
- 不存在 `assistant/chunk`。`assistant/message.data` 与仅日志用途的 `assistant/attempt.data` 都要求内嵌的 `stream`。

[V3 codec](../../../packages/session/session-format-v2-to-v3/src/codec.ts)负责每行一个事件的编码、硬性结构准入，以及包含来源事件范围解码的可恢复规范事件验证。[V2 到 V3 规范](../../../packages/session/session-format-v2-to-v3/README.zh.md#v2-to-v3-specification)负责系统头插入、PTC 与预设名称、序号引用重映射与规范信封转换。
...
| `tokensAtCreate` | 可选 | `number` |
| `updatedAt` | 必需 | `number` |
| `version` | 必需 | `1` |
| `workMsAtCreate` | 可选 | `number` |
...
| `id` | 必需 | `string` |
| `maxGoalRounds` | 必需 | [`event:compaction/end.data.turn`](#persistence-type-eventcompactionenddataturn) |
| `maxGoalTokens` | 必需 | [`event:compaction/end.data.turn`](#persistence-type-eventcompactionenddataturn) |
| `maxGoalWorkMs` | 必需 | [`event:compaction/end.data.turn`](#persistence-type-eventcompactionenddataturn) |
| `objective` | 必需 | `string` |
...
[仅使用源码的提取器](../../../scripts/persistence-schema.ts)以 TypeScript 6.0.3 运行。重建时省略冗余的 `object` 交叉类型成员，并丢弃规范化后不可达的类型定义；保留已有的 `any` / `unknown`，不以不透明类型替代未解析的导入。归档命令从该工作树的 V4 前 schema 冻结本目录，因此其根类型对应 `dsh-v0.1.6-alpha.1` 源码树加上上文提到的同版本追加。
